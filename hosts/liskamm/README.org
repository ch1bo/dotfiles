...
** Replacing a disk in the ZFS pool (e.g. SSD upgrade)
...
Liskamm sits on a residential connection with a dynamic IP. Public traffic for
=ncoding.li= (and friends) hits the VPS at =vpn.ncoding.li= and is forwarded
across a persistent wireguard tunnel to =10.5.5.2= here. The client side is
declared in =wireguard.nix=; the matching peer entry and the DNAT/reverse-proxy
rules live on the VPS.

** First-time key install

Keys are generated out-of-band and dropped into =/root/keys/wg/= (same pattern
as the borg credentials above). Do this once as root on liskamm:

#+begin_src
mkdir -p /root/keys/wg
chmod 0700 /root/keys/wg
umask 077
wg genkey | tee /root/keys/wg/private | wg pubkey > /root/keys/wg/public
chmod 0400 /root/keys/wg/private
#+end_src

Hand =/root/keys/wg/public= to the VPS admin and add it as a peer there with
=AllowedIPs = 10.5.5.2/32=. Paste the VPS's public key into the =publicKey=
field in =wireguard.nix= and rebuild.
...
A healthy peer shows a recent handshake (under ~2 min) and traffic counters
moving. From liskamm, =ping 10.5.5.1= (or whatever the VPS holds) should work;
from the VPS, =ping 10.5.5.2= should work.

** Rotating keys

#+begin_src
wg genkey | tee /root/keys/wg/private.new | wg pubkey > /root/keys/wg/public.new
#+end_src

Update the peer entry on the VPS to the new public key, then on liskamm:

#+begin_src
mv /root/keys/wg/private.new /root/keys/wg/private
mv /root/keys/wg/public.new /root/keys/wg/public
systemctl restart wireguard-wg0
#+end_src

** Changing the tunnel IP or VPS endpoint

Both sides must agree. Update =ips= in =wireguard.nix= and =AllowedIPs= for the
liskamm peer on the VPS in the same change window, otherwise the server will
silently drop packets from the unexpected source.

* Scan inbox

Guest-writable SMB share at =\\liskamm\inbox= (see =inbox.nix=) backed by
=/data/inbox=, bind-mounted into Nextcloud as =/inbox= and surfaced via the
*External Storage* app. Files are owned by uid 33 (=www-data=) so both samba
and Nextcloud can write them.

** One-time Nextcloud setup

1. *Apps* → enable =External storage support=.
2. *Settings → Administration → External storages* → add a =Local= folder at
   =/inbox=, available for user =ch1bo=.

** Verifying from another Linux box

#+begin_src
smbclient -N //liskamm/inbox -c 'put /etc/hostname test.txt'
ls -la /data/inbox/   # on liskamm: owner should be www-data
#+end_src
